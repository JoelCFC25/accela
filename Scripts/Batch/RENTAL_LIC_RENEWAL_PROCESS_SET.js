/*------------------------------------------------------------------------------------------------------/
| Program: RENTAL_LIC_RENEWAL_PROCESS_SET.js  Trigger: Batch
| Client: City of Roseville
|
| Version 1.0 - Base Version. 08/07/2018
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var emailTemplate = "RENTAL_LIC_EXPIRING";					// email Template
...
if (emailAddress.length)
	aa.sendMail("permits@cityofroseville.com", emailAddress, "", ScriptName + " Results", emailText);
...
function mainProcess()
	...
	for(var i=0; i < SetMemberArray.length; i++)
	{
	  ...
	  capId = aa.cap.getCapID(id.getID1(), id.getID2(),id.getID3()).getOutput();
		...
		if (!capId)
			{
			logDebug("Could not get a CapID for " + id.getID1() + "-" + id.getID2() + "-" + id.getID3());
			continue;
		}
	...
	aa.env.setValue("ScriptReturnMessage", "Update Set successful - Rental License Renewal Process Script");
